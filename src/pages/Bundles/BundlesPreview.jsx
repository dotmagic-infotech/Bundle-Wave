                            ...
                            {type === "mixmatchDeal" && (
                                <>
                                            ...
                                            <div key={index}>
                                                <div style={{ display: "flex", padding: "15px 10px" }}>
                                                    ...
                                                    {value?.collection?.length > 0 &&
                                                        <img src={value?.collection?.length > 0 ? value?.collection[0]?.image : value?.products[0]?.image ? value?.media?.[0] : value?.media?.[0]} style={{ width: "60px", height: "60px", marginLeft: "10px", objectFit: "fill" }} />
                                                    }
                                                    {value?.products?.length > 0 &&
                                                        <img src={value?.products[0]?.image || value?.media[0]} style={{ width: "60px", height: "60px", marginLeft: "10px", objectFit: "fill" }} />
                                                    }
                                                    <div style={{ marginLeft: "10px" }}>
                                    ...
                                    <div style={{ display: "flex", justifyContent: "center" }}>
                                        <button style={{
                                            backgroundColor: "#262626", color: "white", cursor: "pointer", width: "45px", height: "45px", borderRadius: "50%", padding: "8px",
                                            fontWeight: "500", fontSize: "20px", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1
                                        }}>+</button>
                                    </div>
                                    <div style={{ border: "1px solid gray", borderRadius: "10px", display: "flex", flexDirection: "column", gap: "0.5rem", marginTop: "-6px" }}>
                                        {getY.map((value, index) => (
                                            <div key={index}>
                                                <div style={{ display: "flex", padding: "15px 10px", position: "relative", overflow: "hidden" }}>
                                                    <div style={{ cursor: "pointer", display: 'flex' }}>
                                                        {selectedSecond === "" ?
                                                            <Button icon={ChevronRightIcon} variant="plain" onClick={() => setSelectedSecond(index)}></Button>
                                                            :
                                                            <Button icon={ChevronDownIcon} variant="plain" onClick={() => setSelectedSecond("")}></Button>
                                                        }
                                                    </div>
                                                    {value?.collection?.length > 0 &&
                                                        <img src={value?.collection?.length > 0 ? value?.collection[0]?.image : value?.products[0]?.image ? value?.media?.[0] : value?.media?.[0]} style={{ width: "60px", height: "60px", marginLeft: "10px", objectFit: "fill" }} />
